...
# jsp에서 에러 처리
1. jsp는 servlet 클래스로 컴파일되기 때문에, 자바관련 여러에러가 페이지에서 발생한다.
	이 때문에 페이지 단위로 에러에 대한 처리가 필요하고, 일반적으로 에러페이지(에러가 발생시 대처페이지)를 화면으로
...
2. 에러가능성이 있는 페이지 구현
	1) directive : errorPage="선언한에러페이지"
# 서버당 에러 처리
1. 응답코드와 해당 클래스 확인
2. web.xml에 error-page로 태그로
	지정할 에러페이지 지정
3. error-page 선언.
