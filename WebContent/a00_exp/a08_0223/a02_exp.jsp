...
[하] 1. jstl의 forEach varStatus의 상태 속성에 대하여 기술하세요.
	...
	sts.first : 반복문에서 해당 단위데이터가 첫번째인지 여부를 boolean으로
				가져온다.
	sts.last : 반복문에서 해당 단위데이터가 마지막째인지 여부를 boolean으로
				가져온다.
--%>
<%-- 
...
--%>
	...
	<form method="post">
	<table>
		<tr><th>제품명</th><td><input type="text" name="name" value="${param.name}" /></td></tr>
	...
	<jsp:useBean id="prod" class="jspexp.z01_vo.Product3"/>
	<jsp:setProperty property="name" name="prod" value="${(empty param.name)?'':param.name}"/>
	<jsp:setProperty property="fr_price" name="prod" value="${(empty param.fr_price)?0:param.fr_price}"/>
	<jsp:setProperty property="to_price" name="prod" value="${(empty param.to_price)?99999:param.to_price}"/>
		...
		$('[name=come]').val("이쪽으로 와보세요.").mouseenter(function(){
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					$('#result').append("<h3>"+xhr.responseText+"</h3>");
