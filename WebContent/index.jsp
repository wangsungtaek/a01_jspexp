...
1. 구성
http://www.google.com/search?h1=en&q=jsp&oq=call
http://192.168.0.23:7080/member/login.jsp?id=himan&pass=7777
	...
	2) 구성 내용
		프로토콜 : 네트웍상 패킷(데이터전송단위) 전송 방식
			...
			전송하는 프로토콜이다.
			ex) ftp: 파일전송 프로토콜, smtp, pop
		호스트 : 접속할 서버의 ip 위치 도메인을 통해서 연동한다.
		포트 : 해당 ip 서버에서 여러가지 섭;스를 지원할 수 있기에,
		...
		쿼리스트링 : 요청값을 서버에 전달했을 때, 웹 서버에 있는 웹어플리케이션
			컨테이너에서 java프로그램이 해석하여 처리하는 요청값 처리형식을 말한다.
			ex) client에서 요청 : key=value&key=value...
			ps) server 요청값은 request.getParameter("key") ==>
				데이터를 value로 받을 수 있다.
