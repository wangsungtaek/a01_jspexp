...
2. 웹 프로그래밍에서는 java+html의 최종결과값을
	PrintWriter라는 객체를 통해서 stream 방식으로 클라이언트
	브라우저에 전송한다.
3. 기타 쿠키값을 새로만들어 전송하거나, 서버에서 만든 파일을 클라이언트에
	전달(파일다운로드) 등 여러가지 기능을 처리한다.
4. 서버단에서 페이지 호출 처리 메서드.
	...
	cf) 이동할 때, 요청값을 가지고 이동할 수 있다.
		sendRedirect("이동할 페이지?key=요청값&key=요청값");
		location.href=이동할 페이지?key=요청값&key=요청값;
		<a href="이동할 페이지?key=요청값&key=요청값">이동</a>
 --%>
<%
