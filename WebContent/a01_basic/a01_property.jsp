...
1. @ page : 페이지에 대한 선언 Directive라고 한다..
	...
	2) 화면 출력형식 : contentType
		ex) text/html : html, 파일전송, execel, pdf등을 전송할 수 있다.
	3) 문자의 encodingq방식 : pageEncoding
2. html 코드를 기반으로 하고,
3. 스크립트릿(scriptlet), 표현식(expresstion), 선언부(Declaration)를
...
3. 제공 디렉티브이름
	...
	2) taglib : 사용할 태그 라이브러리를 지정.
		ex) %@ taglib uri="@@@.@@"
		jst1: 표준 태그 라이브러리 : 기본 태그로 지정되지 않는 내용을
...
2. 스크립트 요소
	1) 표현식(Expressing) - 변수를 통해 값을 출력 ex) %p=변수명
...
%>
<% // 2. 스크립트릿 : 반복문 조건문 등 여러가지 프로그램을 처리하는 부분.
	// cf) 자바 클래스 main() 영역으로
	int num01 = 0;
	name = "신길동"; // 값의 변경.
	int result = plus(25,30); // 선언한 결과값을 변수에 할당.
	for(int cnt=1;cnt<=10;cnt++){
%>
		<%-- 3.익스프레션 : 위 선언부나 스클립틀릿의 데이터 태그와
			함께 화면에 출력 --%>
		<h3><%=cnt %>번째 안녕하세요</h3>
...
%>
	<%--
	ex) 선언부에는 물건가격과 갯수를 매개변수로 받아 총계를 리턴하는 메서드 선언.
		스클릿트부분에 위 물건명, 가격, 갯수를 할당하고,
		익스프레션 부분에 물건명, 가격, 갯수, 총계는 선언부에 선언한 메서드를
		통해서 처리하여 h3형식으로 출력하세요.
	--%>
<%!
	String pname = "사과";
	int price = 1000;
