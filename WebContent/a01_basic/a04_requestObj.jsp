...
<%-- path 기준으로 모든 자원(css, img, js)를 접근하여 사용할 수 있다.--%>
<link rel="stylesheet"
	herf="<%=path %>/a00_com/a00_com.css">
<style>
...
3. 주요메서드
	...
	4) request.getContextPath();
		위 주소에 해당 하는 context명을 말한다. 웹서버에 등록하여 할당하는 프로젝트명이기도 한다.
		WebContent 폴드를 기본경로로 하여 다른 자원의 호출할 때, 사용된다.
	5) request.getRequestURI();
		웹 브라우저가 요청한 url경로 :
		위 주소에서 context명/폴드/파일.jsp에 해당한다.
	6) request.getServerPort()
		위 주소기준이면 7080에 해당한다.
	7) request.getServerName() 연결할 때, 사용한 서버이름.
...
<body>
	...
	요청정보 프로토콜 IP = <%= request.getProtocol() %> <br>
