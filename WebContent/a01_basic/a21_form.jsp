...
대상 페이지 : action ="요청값을 받는 페이지"
 -->
	...
	<!--
	ex) loc=서울강남역&num01를 전송하는 속성지정해서 등록2를 처리하세요.
	 -->
	<form action="a20_request.jsp"> 
	<table>
		<tr><th>장소</th>
...
ex) 받는 a21_reqExp.jsp
 ...
 -->
	<form action="a21_reqExp.jsp" method="get"> 
	<table>
		<tr><th>이름</th>
			<td><input type="text" name="name"/></td></tr>
		<tr><th>나이</th>
...
<!-- 
1. 숫자 데이터에 대한 처리.
	1) 전달 전에 validate 체크로 반드시 입력하고, 숫자형일 때만 전달되게 js처리한 후
		전송되게 한다.
 -->
	<form action="a22_reqExp.jsp" id="frm" method="get"> 
	<table>
		<tr><th>숫자1</th>
			<td><input type="text" name="num01"/></td></tr>
		<tr><th>숫자2</th>
		...
		<tr><td colspan="2">
			<input type="button" onclick="valid()" value="등록"/></td></tr>
	</table>
	</form>
	<h3>다중 데이터 처리.</h3>
	<%-- 
	...
	--%>
	<form action="a23_reqExp.jsp" id="frm2" method="get"> 
	<table>
		<tr><th>물건명</th>
...
<script type="text/javascript">
	function valid(){
		// #frm [name=num01] : id값으로 frm인 요소객체의 계층레벨에 상관없이 하위에
		// 속성이 name이고 속성값 num01인 객체를 선택..
		...
		if(num01Obj.value == ""){
			...
			if(isNaN(num01Obj.value)){ // 숫자형태가 아니면.
				...
				// js에의해 전송처리.(form하위에 submit()버튼을 누른 것과 동일)
